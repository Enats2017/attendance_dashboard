...
export default function DeptAttendance() {
  // Default to current month/year and show the full month on first load
  ...
  const [deptList, setDeptList] = useState([]); // List of { dept_id, department_name, std_hc }
  const [showSettings, setShowSettings] = useState(false);
  const [editHc, setEditHc] = useState({}); // dept_id -> hc
  ...
  async function fetchStdHc() {
    try {
      ...
      if (data.success && data.data) {
        const map = {};
        data.data.forEach((d) => { map[d.dept_id] = parseInt(d.std_hc, 10); });
        setStdHcMap(map);
  ...
  async function loadReport() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setReportData(data); setLoading(false); return; }
    } catch { }
    const data = generateDummyReport(month, year, dayFrom, dayTo, stdHcMap);
    setReportData(data);
    setLoading(false);
  }
  ...
  // addDepartment and deleteDepartment are removed as departments are now dynamic from SQL Server
  ...
  return (
    <div className="da-page">
      {toast && <div className={`da-toast da-toast-${toast.type}`}>{toast.type === 'success' ? '✅' : toast.type === 'error' ? '⚠️' : 'ℹ️'} {toast.msg}</div>}

      <div className="da-header">
        <div className="da-header-logo">🏭</div>
        <div><h1>Department Attendance</h1><p>Per Day Actual Head Count (HC) Report</p></div>
        <div className="da-header-actions"><a href={DASH_URL} className="da-btn-back">⬅ Back to Dashboard</a></div>
      </div>

      <div className="da-filter-bar">
        <div className="da-filter-group"><label>Month</label>
          <select value={month} onChange={(e) => setMonth(Number(e.target.value))}>
            {MONTH_NAMES.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
          </select></div>
        <div className="da-filter-group"><label>Year</label>
          <select value={year} onChange={(e) => setYear(Number(e.target.value))}>
            {[2024, 2025, 2026, 2027, 2028].map((y) => <option key={y} value={y}>{y}</option>)}
          </select></div>
        <div className="da-filter-group"><label>Day From</label>
          <input type="number" min={1} max={maxDays} value={dayFrom} onChange={(e) => setDayFrom(Math.max(1, Math.min(Number(e.target.value), maxDays)))} /></div>
        <div className="da-filter-group"><label>Day To</label>
          <input type="number" min={1} max={maxDays} value={dayTo} onChange={(e) => setDayTo(Math.max(1, Math.min(Number(e.target.value), maxDays)))} /></div>
        <div className="da-filter-group"><label>&nbsp;</label>
          <button className="da-btn da-btn-sm" style={{ background: '#e2e8f0', color: '#475569' }} onClick={() => { setDayFrom(1); setDayTo(maxDays); }}>📅 Full Month</button></div>
        <div className="da-filter-actions">
          <button className="da-btn da-btn-primary" onClick={loadReport}>🔍 Generate</button>
          <button className="da-btn da-btn-warning" onClick={() => { setEditHc({ ...stdHcMap }); setShowSettings(true); }}>⚙️ STD HC Settings</button>
          <button className="da-btn da-btn-success" onClick={exportExcel}>⬇ Excel</button>
          <button className="da-btn da-btn-sm" style={{ background: '#475569', color: '#fff' }} onClick={() => window.print()}>🖨 Print</button>
        </div>
      </div>

      <div className="da-report-container">
        {loading ? <div className="da-loading"><div className="da-spinner" /><p>Generating report…</p></div>
         : reportData ? <ReportTable data={reportData} days={days} unitName={unitName} unitCapacity={unitCapacity} month={month} year={year} />
         : <div className="da-loading"><p>No data available.</p></div>}
      </div>
...
function ReportTable({ data, days, unitName, unitCapacity, month, year }) {
  ...
  return (
    <div className="da-report-card">
      <div className="da-report-title">{unitName} Unit Per Day Actual Head Count (HC) Report {MONTH_NAMES[month - 1]} {year}</div>
      <div className="da-table-scroll">
        <table className="da-table" id="da-report-table">
          <thead><tr><th>Department</th><th>STD<br />({unitCapacity})</th>{days.map((d) => <th key={d}>{d}</th>)}<th>Avg.<br />HC</th></tr></thead>
          <tbody>
            {departments.map((dept, i) => (
              <tr key={i}><td>{dept.department}</td><td>{dept.std_hc}</td>
                {days.map((d) => { 
                  const val = (dept.days && dept.days[d] !== undefined) ? dept.days[d] : ''; 
                  return <td key={d} className={val === 0 || val === '' ? 'da-empty' : ''}>{val === 0 ? '' : val}</td>; 
                })}
                <td>{dept.avg_hc}</td></tr>
            ))}
          </tbody>
          <tfoot>
            <SummaryRow label="Total Present For The Day" stdValue={total_std_hc} dayValues={summary.total_present} avgValue={summary_avg.total_present} days={days} className="da-row-total" />
...
function SummaryRow({ label, stdValue, dayValues, avgValue, days, className }) {
  return (
    <tr className={className}><td>{label}</td><td>{stdValue}</td>
      {days.map((d) => { 
        const val = (dayValues && dayValues[d] !== undefined) ? dayValues[d] : ''; 
        return <td key={d} className={val === 0 ? 'da-empty' : ''}>{val === 0 ? '' : val}</td>; 
      })}
      <td>{avgValue || 0}</td></tr>
  );
}
  ...
  return (
    <div className="da-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="da-modal">
        <div className="da-modal-header"><h2>⚙️ STD Head Count Settings</h2><button className="da-modal-close" onClick={onClose}>✕</button></div>
        ...
        <div className="da-settings-table-wrap">
          <table className="da-settings-table"><thead><tr><th style={{ width: '15%' }}>ID</th><th style={{ width: '60%' }}>Department Name</th><th style={{ width: '25%' }}>STD HC</th></tr></thead>
            <tbody>{deptList.map((dept) => (
              <tr key={dept.dept_id}>
                <td style={{ fontSize: 11, color: '#94a3b8' }}>{dept.dept_id}</td>
                <td style={{ fontWeight: 600 }}>{dept.department_name}</td>
                <td><input type="number" min={0} value={editHc[dept.dept_id] || 0} onChange={(e) => setEditHc((prev) => ({ ...prev, [dept.dept_id]: parseInt(e.target.value, 10) || 0 }))} /></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        <div className="da-settings-total"><span>Total STD Head Count</span><span>{total}</span></div>
        <div className="da-modal-footer">
          <button className="da-btn da-btn-sm" style={{ background: '#e2e8f0', color: '#475569' }} onClick={onClose}>Cancel</button>
          <button className="da-btn da-btn-sm da-btn-primary" onClick={onSave}>💾 Save Changes</button>
        </div>
      </div>
    </div>
  );
