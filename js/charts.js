...
var Charts = (function () {
    ...
    var baseYaxis = {
        labels: {
            style: { fontSize: '11px', fontWeight: 500, colors: '#94a3b8', fontFamily: font }
        }
    };
    ...
    // ── BAR CHART ────────────────────────────────────────────────────────
    function bar(id, categories, series, title, horizontal, onDataClick) {
        destroy(id);
        var el = document.getElementById(id);
        if (!el) return;

        var isSeries = Array.isArray(series[0]);
        var seriesArr = isSeries ? series : [{ name: title || 'Count', data: series }];
        ...
        var opts = {
            ...
            plotOptions: {
                bar: {
                    ...
                    columnWidth: categories.length <= 3 ? '35%' : categories.length <= 6 ? '45%' : '58%',
            ...
            colors: palette,
            fill: isSeries
                ? { type: 'gradient', gradient: { shade: 'light', type: 'vertical', shadeIntensity: 0.2, opacityFrom: 0.95, opacityTo: 0.75 } }
                : { type: 'gradient', gradient: { shade: 'light', type: 'vertical', shadeIntensity: 0.35, opacityFrom: 1, opacityTo: 0.72, stops: [0, 100] } },
            dataLabels: {
                enabled: !horizontal,
                style: { fontSize: '11px', fontWeight: 800, fontFamily: font, colors: ['#1e293b'] },
                offsetY: -6,
    ...
    function donut(id, labels, series, title, onDataClick) {
        ...
        var opts = {
            ...
            dataLabels: {
                enabled: true,
                style: { fontSize: '11px', fontWeight: 800, fontFamily: font, colors: ['#fff'] },
                ...
                pie: {
                    donut: {
                        ...
                        labels: {
                            show: true,
                            name: {
                                show: true, fontSize: '13px', fontWeight: 700,
                                color: '#64748b', fontFamily: font,
                                offsetY: -8
                            },
                            value: {
                                show: true, fontSize: '28px', fontWeight: 900,
            ...
            tooltip: Object.assign({}, baseTooltip, {
                y: {
                    formatter: function (val) { return val + ' (' + (total ? (val * 100 / total).toFixed(1) : 0) + '%)'; }
                }
            }),
            states: {
    ...
    function line(id, categories, seriesArr, title, onDataClick) {
        ...
        var events = onDataClick ? {
            ...
            markerClick: function (e, ctx, cfg) {
                if (onDataClick) onDataClick(categories[cfg.dataPointIndex], cfg.dataPointIndex, cfg.seriesIndex);
            }
        } : {};
        ...
        if (isSingle) {
            ...
            opts = {
                chart: Object.assign({}, baseChart, { type: 'bar', height: 300, events: events }),
                series: seriesArr,
                ...
                fill: {
                    ...
                    gradient: {
                        ...
                        inverseColors: false,
                        shadeIntensity: 0.3, opacityFrom: 1, opacityTo: 0.7
                    }
                },
                dataLabels: {
                    enabled: true,
                    style: { fontSize: '16px', fontWeight: 900, fontFamily: font, colors: ['#1e293b'] },
                    offsetY: -10,
                    background: { enabled: false }
                },
                grid: baseGrid,
                ...
                annotations: {
                    yaxis: [{
                        y: 0, borderColor: 'transparent',
                        label: {
                            text: '📅 Single day view',
                            position: 'left',
                            style: { color: '#94a3b8', fontSize: '10px', fontFamily: font, background: 'transparent', border: 0 }
                        }
                    }]
                }
            };
            ...
            opts = {
                chart: Object.assign({}, baseChart, { type: 'area', height: 300, events: events }),
                series: seriesArr,
                xaxis: Object.assign({}, baseXaxis, {
                    categories: categories,
                    labels: Object.assign({}, baseXaxis.labels, {
                ...
                grid: baseGrid,
                tooltip: Object.assign({}, baseTooltip, { shared: seriesArr.length > 1, intersect: false }),
    ...
    // ── STACKED BAR ───────────────────────────────────────────────────────
    function stacked(id, categories, seriesArr, title, onDataClick) {
        destroy(id);
        var el = document.getElementById(id);
        if (!el) return;
        ...
        var opts = {
            ...
            plotOptions: {
                bar: {
                    borderRadius: 6,
                    borderRadiusApplication: 'end',
                    columnWidth: categories.length <= 3 ? '32%' : categories.length <= 6 ? '44%' : '58%'
                }
            },
            fill: { opacity: [0.92, 0.80] },
            dataLabels: {
                enabled: true,
                style: { fontSize: '11px', fontWeight: 700, fontFamily: font, colors: ['#fff'] },
                formatter: function (val) { return val > 0 ? val : ''; }
            },
            legend: Object.assign({}, baseLegend, { position: 'top', horizontalAlign: 'right' }),
            grid: baseGrid,
            tooltip: Object.assign({}, baseTooltip, { shared: true, intersect: false }),
            states: {
        ...
        var opts = {
            ...
            plotOptions: {
                radialBar: {
                    ...
                    dataLabels: {
                        name: { fontSize: '12px', fontWeight: 700, color: '#475569', fontFamily: font },
                        value: { fontSize: '16px', fontWeight: 900, color: '#0f172a', fontFamily: font }
                    }
                }
            },
            legend: Object.assign({}, baseLegend),
            fill: { type: 'gradient', gradient: { shade: 'light', type: 'horizontal', shadeIntensity: 0.5, opacityFrom: 1, opacityTo: 0.85 } },
