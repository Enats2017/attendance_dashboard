...
class AttendanceView {
    ...
    _renderSummaryCards(stats) {
        const cards = [
            { label: 'Present', val: stats.present, icon: 'ph-check-circle', cls: 'success' },
            { label: 'Absent', val: stats.absent, icon: 'ph-x-circle', cls: 'danger' },
            { label: 'Single Punch', val: stats.singlePunch, icon: 'ph-lightning', cls: 'warning' },
            { label: 'Late In', val: stats.lateIn, icon: 'ph-clock-afternoon', cls: 'info' },
            { label: 'Early Out', val: stats.earlyOut, icon: 'ph-sign-out', cls: 'accent' },
            { label: 'Avg Hours', val: stats.avgHours + 'h', icon: 'ph-timer', cls: '' },
    ...
    _countBy(arr, keyFn) {
        const out = {};
        arr.forEach(x => { const k = keyFn(x); out[k] = (out[k] || 0) + 1; });
        return out;
    }
        ...
        return {
            ...
            renderCharts: () => {
                ...
                if (inCount === 0 && outCount === 0) {
                    ...
                    if (el) el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#94a3b8;font-size:14px;font-weight:600;">No data available</div>';
                } else {
    ...
    _renderAll(logs, emps, empMap, filters) {
        ...
        return {
            ...
            renderCharts: () => {
                Charts.line('ch-all-trend', dates, [{ name: 'Total Punches', data: counts }], 'Daily Attendance', date => {
                    this._renderDrillDown(logs.filter(l => l.date === date), `Date: ${date}`, empMap);
                });
                Charts.donut('ch-all-dept', Object.keys(byDept), Object.values(byDept), 'Dept Distribution', dept => {
                    this._renderDrillDown(logs.filter(l => (empMap[l.empId] || {}).dept === dept), `Department: ${dept}`, empMap);
                });
            }
        };
    }
    ...
    _renderDrillDown(logs, title, empMap) {
        ...
        const rows = logs.map(l => {
            const e = empMap[l.empId] || {};
            return `<tr><td><b>${e.code || '–'}</b></td><td>${e.name || '–'}</td><td>${e.dept || '–'}</td><td>${e.company || '–'}</td><td>${l.date}</td><td>${l.inTime || '–'}</td><td>${l.outTime || '–'}</td><td><b>${l.hoursWorked || 0}h</b></td><td>${l.lateIn ? 'Yes' : 'No'}</td><td>${l.earlyOut ? 'Yes' : 'No'}</td><td>${l.status}</td></tr>`;
        }).join('');
        this._drillData = logs.map(l => {
            const e = empMap[l.empId] || {};
            return { Code: e.code, Name: e.name, Dept: e.dept, Company: e.company, Date: l.date, In: l.inTime, Out: l.outTime, Hours: l.hoursWorked, Late: l.lateIn, Early: l.earlyOut, Status: l.status };
        });
        container.innerHTML = `<div class="drilldown-box"><div class="drilldown-header"><span>🔍 ${title} <small>(${logs.length} records)</small></span><div style="display:flex;gap:8px"><button class="btn btn-success btn-sm" onclick="AppController.view.exportExcel(AppController.view._drillData, 'drilldown')">⬇ Excel</button><button class="btn btn-sm" onclick="AppController.view.closeDrillDown()">✕ Back</button></div></div><div style="overflow-x:auto"><table class="data-table"><thead><tr><th>Code</th><th>Name</th><th>Dept</th><th>Company</th><th>Date</th><th>In</th><th>Out</th><th>Hours</th><th>Late</th><th>Early</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
        container.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    _renderAgeWise(logs, emps, empMap, model) {
        const groups = ['Under 25', '25–34', '35–44', '45–54', '55+'];
        const gC = {}; const gP = {}; groups.forEach(g => { gC[g] = 0; gP[g] = 0; });
        const seen = new Set();
        emps.forEach(e => { const g = model.getAgeGroup(e.dob); gC[g]++; });
        logs.forEach(l => { if (!seen.has(l.empId)) { seen.add(l.empId); const e = empMap[l.empId]; if (e) { const g = model.getAgeGroup(e.dob); gP[g]++; } } });
        const rows = groups.map(g => [g, gC[g], gP[g], gC[g] - gP[g], gC[g] ? Math.round(gP[g] / gC[g] * 100) + '%' : '0%']);
        this._lastData['age-wise'] = rows.map(r => ({ AgeGroup: r[0], Total: r[1], Present: r[2], Absent: r[3], Rate: r[4] }));
        return {
            html: `<h2 class="section-title"><i class="ph-fill ph-users-three"></i> Age-Wise Analysis</h2><div class="charts-grid">${this._chartCard('ch-age-bar', '<i class="ph-fill ph-chart-bar"></i>', 'amber', 'Attendance by Age Group')}</div>${this._tableHTML('tbl-age', ['Age Group', 'Total Emp', 'Present', 'Absent', 'Attendance %'], rows, 'age-wise')}<div id="drilldown-table" style="margin-top:16px"></div>`,
            renderCharts: () => {
                Charts.stacked('ch-age-bar', groups, [{ name: 'Present', data: groups.map(g => gP[g]) }, { name: 'Absent', data: groups.map(g => gC[g] - gP[g]) }], 'Age-wise', g => {
                    this._renderDrillDown(logs.filter(l => model.getAgeGroup((empMap[l.empId] || {}).dob) === g), `Age: ${g}`, empMap);
                });
            }
        };
    }
    ...
    _renderDeptWise(logs, emps, empMap, model) {
        ...
        const rows = depts.map(d => {
            const t = (eBD[d] || []).length, ls = lBD[d] || [];
            const p = new Set(ls.map(l => l.empId)).size;
            const avg = ls.length ? (ls.reduce((s, l) => s + (l.hoursWorked || 0), 0) / ls.length).toFixed(1) : 0;
            return [d, t, p, t - p, avg, t ? Math.round(p / t * 100) + '%' : '0%'];
        });
        this._lastData['dept-wise'] = rows.map(r => ({ Dept: r[0], Total: r[1], Present: r[2], Absent: r[3], AvgHours: r[4], Rate: r[5] }));
        return {
            html: `<h2 class="section-title"><i class="ph-fill ph-briefcase-metal"></i> Department Statistics</h2><div class="charts-grid">${this._chartCard('ch-dept-bar', '<i class="ph-fill ph-chart-bar"></i>', 'violet', 'Present by Dept')}</div>${this._tableHTML('tbl-dept', ['Dept', 'Total', 'Present', 'Absent', 'Avg Hours', 'Rate'], rows, 'dept-wise')}<div id="drilldown-table" style="margin-top:16px"></div>`,
            renderCharts: () => {
                Charts.bar('ch-dept-bar', depts, rows.map(r => r[2]), 'Dept Attendance', true, d => {
                    this._renderDrillDown(logs.filter(l => (empMap[l.empId] || {}).dept === d), `Dept: ${d}`, empMap);
                });
            }
        };
    }
    ...
    _renderLateEarly(logs, emps, empMap) {
        ...
        const rows = filtered.slice(0, 100).map(l => {
            const e = empMap[l.empId] || {};
            return [e.name, e.dept, l.date, l.inTime, l.outTime, l.lateIn ? 'Yes' : '-', l.earlyOut ? 'Yes' : '-'];
        });
        return {
    ...
    _renderSpecial(logs, emps, empMap, filters, model) {
        ...
        const spRows = singlePunch.slice(0, 100).map(l => { const e = empMap[l.empId] || {}; return [e.name, e.dept, l.date, l.inTime]; });
        return {
