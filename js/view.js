...
class AttendanceView {
  ...
  _renderAgeWise(logs, emps, empMap, model) {
    ...
    const rows = groups.map((g) => [
      ...
      gP[g],
      gC[g] - gP[g],
      gC[g] ? Math.round((gP[g] / gC[g]) * 100) + "%" : "0%",
    ]);
    ...
    return {
      ...
      renderCharts: () => {
        Charts.stacked(
          ...
          groups,
          [
            { name: "Present", data: groups.map((g) => gP[g]) },
            { name: "Absent", data: groups.map((g) => gC[g] - gP[g]) },
          ],
          "Age-wise",
          (g) => {
            this._renderDrillDown(
              logs.filter(
                (l) => model.getAgeGroup((empMap[l.empId] || {}).dob) === g,
              ),
              `Age: ${g}`,
              empMap,
            );
  ...
  _renderDeptWise(logs, emps, empMap, model) {
    ...
    const rows = depts.map((d) => {
        const deptEmps = eBD[d] || [];
        const t = deptEmps.length;
        const ls = lBD[d] || [];

        // ✅ Company wise jaisa same logic
        const empPresentMap = {};
        ls.forEach((l) => {
            const pval = parseFloat(l.present);
            const existing = empPresentMap[l.empId];
        ...
        // Absent = present=0 wale + jo logs mein hain hi nahi
        const loggedAbsent = Object.keys(empPresentMap).filter(
            (id) => empPresentMap[id] === 0
        ).length;
        const noLogEmps = deptEmps.filter(
            (e) => empPresentMap[e.id] === undefined
        ).length;
        const ab = loggedAbsent + noLogEmps;

        const avg = ls.length
            ? (ls.reduce((s, l) => s + (l.hoursWorked || 0), 0) / ls.length).toFixed(1)
            : 0;

        return [d, t, p, hp, ab, avg, t ? Math.round((p / t) * 100) + "%" : "0%"];
    });

    this._lastData["dept-wise"] = rows.map((r) => ({
        Dept: r[0], Total: r[1], Present: r[2], HalfPresent: r[3],
        Absent: r[4], AvgHours: r[5], Rate: r[6],
    }));
    ...
    return {
        html: `<h2 class="section-title"><i class="ph-fill ph-briefcase-metal"></i> Department Statistics</h2>
            ...
            ${this._tableHTML("tbl-dept", ["Dept", "Total", "Present", "Half Present", "Absent", "Avg Hours", "Rate"], rows, "dept-wise")}
            <div id="drilldown-table" style="margin-top:16px"></div>`,
        renderCharts: () => {
            Charts.stacked(
                "ch-dept-bar",
                depts,
                [
                    { name: "Present", data: rows.map((r) => r[2] + r[3]) }, // Present + Half
                    { name: "Absent", data: rows.map((r) => r[4]) },
                ],
                "Dept Attendance",
                (d) => {
                    this._renderDrillDown(
                        logs.filter((l) => (empMap[l.empId] || {}).dept === d),
                        `Dept: ${d}`,
                        empMap,
                    );
                },
  ...
  _renderShiftWise(logs, emps, empMap, model) {
    ...
    const lBS = model.groupBy(
      logs,
      (l) => (empMap[l.empId] || {}).shift || "Unknown",
    );
