...
class AttendanceView {
    ...
    if (seg) {
      ...
      const filteredLogs = dayLogs.filter((l) => {
        switch (status) {
          case "Present":
            return (
              l.status === "Present" || l.status === "Present On WeeklyOff"
            );
          case "Half Present":
            return (
              l.status === "1/2Present" ||
  ...
  _showDeptAccTooltip(e, label, stats) {
    ...
    tip.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#10b981;display:inline-block;"></span> Present: <b style="margin-left:auto;">${stats.present}</b></div>
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#f59e0b;display:inline-block;"></span> Half Present: <b style="margin-left:auto;">${stats.half}</b></div>
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;"><span style="width:8px;height:8px;border-radius:50%;background:#3b82f6;display:inline-block;"></span> Weekly Off: <b style="margin-left:auto;">${stats.weeklyOff}</b></div>
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;"><span style="width:8px;height:8px;border-radius:50%;background:#f43f5e;display:inline-block;"></span> Absent: <b style="margin-left:auto;">${stats.absent}</b></div>
      <div style="display:flex;align-items:center;gap:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.15);"><span style="width:8px;height:8px;border-radius:50%;background:#94a3b8;display:inline-block;"></span> Total: <b style="margin-left:auto;">${stats.total}</b></div>
    `;
    ...
    emps.forEach((e) => {
      dates.forEach((date) => {
        ...
        if (log) {
          ...
          // Koi record nahi mila -> synthetic Absent row
          result.push({
            empId: e.id,
            date: date,
            inTime: null,
...
.dept-accordion.dimmed .dept-acc-seg.active-highlight { opacity: 1; }
      ...
      renderCharts: () => {
        ...
        legendItems.forEach((item) => {
          item.addEventListener("mouseenter", () => {
            const status = item.dataset.legend; // present, half, weeklyoff, absent
  ...
  _renderDeptDesignationDrilldown(dept, emps, logs, model, anchorEl) {
    ...
    // Purana popover hata do agar khula hai
